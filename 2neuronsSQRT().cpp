...
double predict(double x, double w1, double b1, double w2, double b2, double c) {
    // Layer 1: f1(x) = xw1 + b1
    double layer1_output = x * w1 + b1;
    
    // Layer 2: f2(x) = (f1(x) / b2) + c
    return (layer1_output / b2) + c;
}

// Cost function
double cost(double w1, double b1, double w2, double b2, double c) {
    double MSE = 0.0;

    for (size_t i = 0; i < train_count; ++i) {
        double x = train[i][0];
        double y = predict(x, w1, b1, w2, b2, c);
...
void update(int epoch) {
    ...
    double init_cost = cost(w1, b1, w2, b2, c);
    ...
    for (size_t e = 1; e <= epoch; ++e) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            double layer1_output = x * w1 + b1;
            double y_pred = (layer1_output / b2) + c; // Two-layer prediction
            double error = y_pred - y_true;

            // Gradient calculations
            grad_w1 += (2.0 / n) * error * (x / b2);
            grad_b1 += (2.0 / n) * error * (1.0 / b2);
            grad_b2 -= (2.0 / n) * error * (layer1_output / (b2 * b2));
            grad_c += (2.0 / n) * error;
        }
        ...
        double current_cost = cost(w1, b1, w2, b2, c);
    ...
    if (cost(backup_w1, backup_b1, backup_w2, backup_b2, backup_c) < cost(w1, b1, w2, b2, c)) {
    ...
    std::cout << "Final Parameters : w1->" << w1 << ", b1->" << b1 << ", w2->" << w2 << ", b2->" << b2 << ", c->" << c << std::endl;

    // Test prediction
    int input = 42;
    double y_pred_bonus = predict(input, w1, b1, w2, b2, c);
